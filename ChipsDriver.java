...
public class ChipsDriver{
  ...
  {
    SimpleChips player1; // Declares a player1 and player2 object. Because these objects extend SimpleChips (they are a type of SimpleChip)
    SimpleChips player2; // we know we are setting aside enough room no matter which student's object it is.
    int initpile = 100; 
    int pile = 100;
    int lastMove = 0;
    int numgames = 0;
    ...
    ArrayList<SimpleChips> players = new ArrayList<SimpleChips>(); // These are ArrayLists to store all of the potential players
    ArrayList<SimpleChips> players2 = new ArrayList<SimpleChips>(); // Make sure the class files are located in the same folder as this driver file.
    
    //*
    //add new players here in the same format
    players.add(new HumanChips());
    players2.add(new HumanChips());
    players.add(new SimpleChips());
    players2.add(new SimpleChips());
    players.add(new BobbyChips());
    players2.add(new BobbyChips());
    players.add(new KiangChips());
    players2.add(new KiangChips());
    players.add(new ChristianChips());
    players2.add(new ChristianChips());
    players.add(new EzraChips());
    players2.add(new EzraChips());
    players.add(new IsaiahChips());
    players2.add(new IsaiahChips());
    players.add(new JustinChips());
    players2.add(new JustinChips());
    players.add(new WilliamChips());
    players2.add(new WilliamChips());
    players.add(new KylerChips());
    players2.add(new KylerChips());
    players.add(new MicheleChips());
    players2.add(new MicheleChips());
    players.add(new LeoChips());
    players2.add(new LeoChips());
    players.add(new MarissaChips());
    players2.add(new MarissaChips());
    players.add(new BrendansChips());
    players2.add(new BrendansChips());
    players.add(new MasaChips());
...
//*** START OF EACH GAME ***
      while (pile > 0) 
      {
        ...
        if (player1turn) // If it's player1's turn call player1 move
        {
          ...
          nextPlay = player1.play(pile, lastMove); // This calls the Player object's play() method passing in the pile size and the last move. A lastMove value of 0 signifies to the player that there was no previous move
        ...
        else
        {
          ...
          nextPlay = player2.play(pile, lastMove); // otherwise call player2 move
          if (showText)
            System.out.println(p2name+" took " + nextPlay + " chips.");
        }
        // If any illegal moves occurred...
        if ((lastMove > 0 && nextPlay > 2 * lastMove) || // If it's not the first move, and the player takes more than twice what the last player took, or
            (lastMove == 0 && nextPlay > pile - 1) || // if it's the first move and the player tries to take it all, or
